...
class PermutationDeprecatedMixin:
    ...
    def _ascii_plot(self):
        ...
        for i in range(n):
            array[self[i]][i] = "*"
        array.reverse()
        s = "\n".join((" ".join(val) for val in array))
        print(s)

    @deprecated
    def greedy_sum(self):
        """
        This provides a sum-decomposition of `self` in which consecutive increasing
        sum-components are merged.
        """
        parts = []
        sofar = 0
        while sofar < len(self):
            if len(self) - sofar == 1:
                parts.append(Permutation(1))
                return parts
            i = 1
            while sofar + i <= len(self) and list(self[sofar : sofar + i]) == range(
                sofar, sofar + i
            ):
                i += 1
            i -= 1
            ...
            sofar += i
            i = 2
            while sofar + i <= len(self) and not (
                max(self[sofar : sofar + i]) - min(self[sofar : sofar + i]) + 1 == i
                and min(self[sofar : sofar + i]) == sofar
            ):
                i += 1
            if sofar + i <= len(self):
                parts.append(Permutation(self[sofar : sofar + i]))
            sofar += i
        return parts
    ...
    def all_extensions_track_index(self, track_index):
        ...
        for i in range(0, len(self) + 1):
            for j in range(0, len(self) + 1):
                ...
                vals.insert(j, i - 0.5)
                if j < track_index:
                    L.append((Permutation(vals), track_index + 1))
                else:
                    L.append((Permutation(vals), track_index))
        return L

...
class GeometricGridClass(PermClass):
    ...
    def __init__(self, M, col=None, row=None, max_length=8, generate=True):
        ...
        self.alphabet_size = sum(
            [
                (1 if self.M[i][j] != 0 else 0)
        ...
        for (i, l) in enumerate(self.alphabet):
            for j in range(i, len(self.alphabet)):
                if (
                    self.alphabet[i][0] != self.alphabet[j][0]
                    and self.alphabet[i][1] != self.alphabet[j][1]
                ):
        ...
        for c in range(len(col)):
            for r in range(len(row)):
                if M[c][r] != 0 and M[c][r] != 2 and M[c][r] != col[c] * row[r]:
                    ...
                    raise Exception(
                        "Unable to find row and columns signs. "
...
#    allowed_inflations.extend(
#        list(
#            set(
#                [
#                    P
#                    for sublist in [
#                        B.all_intervals(return_patterns=True) for B in basis
#                    ]
#                    for P in sublist
#                ]
#            )
...
#           if Q in basis:
#               what_to_avoid = [
#                   chr(e + 97) + "_" + str(allowed_inflations.index(components[i]))

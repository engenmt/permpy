...
from permpy import AvClass, Perm, PermClass, PermSet, Permutation
from permpy.RestrictedContainer import RestrictedContainer
...
def check_stat(f, L1, L2, length=8):
    C = [sorted([f(P) for P in T[length]]) for T in L1]
    D = [sorted([f(P) for P in T[length]]) for T in L2]
    return [(i, j) for i in range(len(L1)) for j in range(len(L2)) if C[i] == D[j]]


def check_stat_list(fs, L1, L2, length=8):
    C = [sorted([tuple([f(P) for f in fs]) for P in T[length]]) for T in L1]
    D = [sorted([tuple([f(P) for f in fs]) for P in T[length]]) for T in L2]
    # done = []
    # vals = []
    # for thing in C[0]:
    # if thing in done:
...
def check_stats(L1, L2, length=8, add_syms=False, tups=1):
  ...
  """
    ...
    stats = [
        ...
        # (
        #     "num ltrmax / rtlmin layers",
        #     lambda P: P.reverse().num_rtlmax_ltrmin_layers(),
        # ),
        # (
        #     "num rtlmax / ltrmin layers contracted",
        #     lambda P: P.contract_inc_bonds().num_rtlmax_ltrmin_layers(),
        # ),
    ...
    C1 = [AvClass(B, length) for B in L1]
    C2 = [AvClass(B, length) for B in L2]

    print("")
    if tups == 1:
        for (name, f) in stats:
            equivs = check_stat(f, C1, C2, length)
            print(name, ":")
            if len(equivs) == 0:
                print("\tnone\n")
                continue
        ...
        for stat_set in fs:
            equivs = check_stat_list(
                [stat_set[i][1] for i in range(len(stat_set))], C1, C2, length
            )
            print("")
            print(

...
from permpy import PermSet, Permutation
...
class PermutationMiscMixin:
    ...
    def all_right_extensions(self, max_length, length, S):
        ...
        for p in re:
            S.add(p)
            S = p.all_right_extensions(max_length, length + 1, S)
        return S
    ...
    def sum_indec_bdd_by(self, n):
        ...
        while len(S) > 0 and len(S[0]) > 0:
            lens = [len([s for s in S if not s.sum_decomposable()])] + lens
            if lens[0] > n:
                return False

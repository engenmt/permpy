...
class Permutation(tuple, 
	...
	def bona_good(self):
		"""
		"""
		return self.avoids(Permutation(132)) and (self*self).avoids(Permutation(132))
	...
	for n in range(len(pi),10):
		...
		for tau in L:
			...
			if tau.involves(iota):
				...
				for copy in copies:
					rest_of_tau = [val for idx, val in enumerate(tau) if idx not in copy]
					for sigma in L:
						rest_of_sigma = [val for idx, val in enumerate(sigma) if idx not in copy]
	...
	for n in range(len(iota), height+1):
		...
		for tau in S:
			...
			for copy in copies:
				...
				print("{:30s} {:6d}".format("\tnew = {}".format(lower_inflation), lower_num_copies), end = "")
				...
				print("{:30s} {:6d}".format("\tnew = {}".format(upper_inflation), upper_num_copies))
				sys.stdout.flush()

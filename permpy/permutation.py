...
class Permutation(
    ...
    def random_avoider(cls, n, B, simple=False, involution=False, verbose=-1):
        ...
        Args:
            ...
            simple (Boolean, optional): Whether the returned Permutation should be
                                        simple.
                Defaults to False.
            involution (Boolean, optional): Whether the returned Permutation should be
                                            an involution.
                Defaults to False.
    ...
    def delete(self, indices=None, values=None):
        ...
        Examples:
            ...
            ...     Permutation(35214).delete(indices=[2])
            ...     ==
            ...     Permutation(35214).delete(values=1)
            ... )
            True
    ...
    def restrict(self, indices=None, values=None):
        """
        Return the permutation obtained by restricting self to the given indices or
        values.
        """
    ...
    def pretty_out(self, by_lines=False, width=2):
        ...
        Examples:
            ...
            >>> for line in Permutation([1,9,3,7,5,6,4,8,2,10]).pretty_out(
            ...     by_lines = True
            ... ):
            ...     print(repr(line))
            ...
    ...
    def descents(self):
        ...
        """
        return [
            i for i in range(len(self) - 1) if self[i] >= self[i + 1]
        ]  # >= is a bit faster than > for some reason.
    ...
    def pattern_counts(self, k):
        """
        Return a Counter (dictionary) counting the occurrences of each perm of length
        `k` in `self`.
    ...
    def avoids(self, p=None, lr=0, B=None):
        ...
        Args:
            ...
            B (iterable of permutation-like objects:optional): A collection of
                permutations to be avoided.
    ...
    def plot(self, show=True, ax=None, use_mpl=True, fname=None, **kwargs):
        ...
        back to an ascii_plot if use_mpl is set to False.
        """
        if not use_mpl:
    ...
    def contiguous_copies(self, other):
        """
        Return the list of (indices corresponding to) immediate copies of `other`
        in `self`.
        """
        immediate_copies = []
        m = len(other)
    ...
    def optimizers(self, n):
        """
        Return the list of permutations of length `n` that contain the most possible
        copies of `self`.
        """
        max_copies = 0
        best_perms = []

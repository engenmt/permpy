...
class PegPermutation(Permutation):
    ...
    def __new__(cls, p, signs):
        ...
        assert len(p) == len(signs), f"Incorrect number of signs given: {signs}"
        valid_signs = {'+', '-', '.'}
        assert all(sign in valid_signs for sign in signs), f"Invalid sign(s) given: {signs}"
    ...
    def __eq__(self, other):
        return tuple(self) == tuple(other) and tuple(self.signs) == tuple(other.signs)

    def num_signs(self):
        return len(self.signs.filter(lambda s: s != '.'))
        # return sum([1 for s in self.signs if s != '.'])

    def sign_subset(self, other):
        """Return whether the signs for self are 'contained' in those of other.

        Notes:
            A 'dot' is contained in all signs, and any sign is contained in itself.
        """
        for sign_self, sign_other in zip(self.signs, other.signs):
            if not (sign_self == sign_other or sign_self == '.'):
                return False
        return True

    def filling_vector(self):
        L = [1 if s == '.' else 2 for s in self.signs]
        return Vector(L)
    ...
    def all_dotted_monotone_intervals(self):
        ...
        for i in range(0,len(self)-1):
            if self.signs[i] == '.' and self.signs[i+1] == '.' and math.fabs(self[i] - self[i+1]) == 1 and (c_length == 0 or self[i] - self[i+1] == difference):
                if c_length == 0:
                    c_start = i
                c_length += 1
    ...
    def is_compact(self):
        inc_signs = {('+','.'), ('.','+'), ('+','+')}
        dec_signs = {('-','.'), ('.','-'), ('-','-')}
        for (prev_val, next_val), sign_pair in zip(zip(self, self[1:]), zip(self.signs, self.signs[1:])):
            if prev_val+1 == next_val:
                if sign_pair in inc_signs:
                    return False
            elif prev_val == next_val+1:
                if sign_pair in dec_signs:
                    return False
        return True
    ...
    def is_compact_and_clean(self):
        if not self.is_compact():
            return False
        for i in range(0, len(self)-1):
            if self.signs[i] == '.' and self.signs[i+1] == '.' and fabs(self[i] - self[i+1]) == 1:
                return False
        return True
    ...
    def split(self, indices):
        ...
        for index in indices:
            ...
            if signs[index+offset] == '+':
                new_entries.append(entries[index+offset]-.5)
                new_entries.append(entries[index+offset]+.5)
            else:
                new_entries.append(entries[index+offset]+.5)
                new_entries.append(entries[index+offset]-.5)        

...
from permpy.deprecated.permsetdeprecated import PermSetDeprecatedMixin
from permpy.permmisc import lcm
...
class PermSet(set, PermSetDeprecatedMixin):
    ...
    def __init__(cls, s=[]):
        """
        Return the PermSet from the iterable provided, or just with a single
        permutation.
        """
    ...
    def upset(self, up_to_length):
        ...
        if max_length > up_to_length:
            raise ValueError(
                f"PermSet.upset called with up_to_length = {up_to_length} on a PermSet "
                "with a longer permutation."
            )
    ...
    def pattern_counts(self, k):
        """
        Return a dictionary counting the copies of all `k`-perms in each permutation
        in `self`.
        """
        C = Counter()
        for pi in self:
    ...
    def heatmap(self, only_length=None, ax=None, blur=False, gray=False, **kwargs):
        ...
        Args:
            only_length (int:optional):  If given, restrict to the permutations of
                this length.
        """
